...
# ---------------------------------------------------------
    ...
    if g_f and p_f:
        ...
        # Normalize Trait (Higher is better) and nHomAlt (Lower is usually better for stability)
        df['Merit_Score'] = (df[trait] / df[trait].max()) - (df['nHomAlt'] / df['nHomAlt'].max())
        
        col_x, col_y = st.columns([2, 1])
        with col_x:
            st.subheader("Figure 4: Selection Pressure Map")
            fig4 = px.scatter(df, x="nHomAlt", y=trait, size="Merit_Score", color="Merit_Score",
